...
const sequenceSchema = joi.object().keys({
  ...
  sequence: joi
    ...
    .unique()
    .required()
    .items(joi.string().regex(/^(.*\..*)?\.(changeme|validate)/)), // append create apply update validate delete trigger notify report cancel find renew assert import send unapply remove  (event.update.sfdc__greenfield-request--processed ???)
  catchers: joi.array()
});
