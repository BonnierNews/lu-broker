...
function validate(recipes, schema) {
  const {error} = joi.validate(recipes, schema);
  if (error) {
...
function validateTriggers(triggers) {
  const {error} = joi.validate(triggers, triggerSchema);
  if (error) {
    const message = error.details.map((d) => `value: ${JSON.stringify(d.context.value)} detail: ${d.message}`);
    error.message = message.join(", ");
    throw error;
...
function init(recipes, triggers = {}) {
  ...
  recipes.forEach((recipe) => {
    ...
    if (recipe.unrecoverable) {
      for (const key of [...Object.keys(lambdaMap), ...Object.keys(borrowedKeys)]) {
        unrecoverableMap[key] = recipe.unrecoverable[0]["*"];
      }
    }
  });
