...
const allowedVerbs = ["get-or-create", "get", "update", "upsert", "delete", "validate", "perform"].join("|");
...
const sequenceSchema = joi.object().keys({
  ...
  sequence: joi
    ...
    .unique()
    .required()
    .items(joi.string().regex(new RegExp(`^(.*\\..*)?\\.(${allowedVerbs})`))), // append create apply update validate delete trigger notify report cancel find renew assert import send unapply remove  (event.update.sfdc__greenfield-request--processed ???)
  catchers: joi.array()
});
...
function validate(event) {
  const {error} = joi.validate(event, recipeShema);
  if (error) {
