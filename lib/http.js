...
async function performRequest(method, params) {
  ...
  try {
    ...
    logger.info(`HTTP response for ${method} ${url}, ${response.statusCode}, ${JSON.stringify(response.data)}`);
  } catch (err) {
    logger.warning(
      `HTTP ${method}:${url} yielded ${response && response.statusCode}, error: ${err}, body: ${
