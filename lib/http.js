...
function performRequest(method, params) {
  const logger = buildLogger({meta: {correlationId: params.correlationId, requesterName: callingAppName}});
  ...
  return new Promise((resolve, reject) => {
    ...
    request(opts, (err, response, body) => {
      ...
      logger.debug(`HTTP response for ${method} ${url}, ${response.statusCode}, ${JSON.stringify(response.body)}`);
