...
async function buildHeaders(params, headers = {}) {
  ...
  if (livesInGcp && livesInGcp.includes(application)) {
    const gcpHeader =
      application === "bn-subscriptions"
        ? await getGcpAuthHeaders(config.bnSubscriptionApi.audience)
        : await getGcpAuthHeaders(config.gcpProxy.audience);
...
function getUrl(params) {
  const application = params.path.split("/").find(Boolean);
  if (livesInGcp && livesInGcp.includes(application)) {
    return application === "bn-subscriptions"
      ? `${params.baseUrl || config.bnSubscriptionApi.url}${params.path}`
      : `${params.baseUrl || config.gcpProxy.url}${params.path}`;
  }
