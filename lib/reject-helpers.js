...
async function caseIf(predicate, caseBody) {
  ...
  if (predicate) {
    ...
    rejectUnless(config.salesforceApiUrl, "Missing URL to salesforce-api");
...
function assertCaseBody(caseBody) {
  const requiredCaseProperties = ["namespace", "origin", "subject", "description", "sourceQueue", "owner"];
  const allowedCaseProperties = [
