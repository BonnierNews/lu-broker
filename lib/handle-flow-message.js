    ...
    try {
      try {
        ...
        if (response && response.type === "trigger") {
          await publish(
            `${broker.internalPrefix}.internal.trigger-message`,
            {
              type: "internal-message",
              id: `${routingKey}:${context.correlationId}`,
              attributes: {
                trigger: `trigger.${response.id}`,
          ...
          // TODO: do this in trigger
          // await publish(`trigger.${response.id}`, response.source, {...context.meta, notifyProcessed: true}, context);
        } else {
