...
const workerBehavior = Object.assign(
  {
    confirm: true,
    ack: true,
    logger,
...
function setupBroker(behavior) {
  ...
  broker.on("subscribed", (subscription) => {
    ...
    logger.info(`Subscription started: ${JSON.stringify(subscription, getCircularReplacer())}`);
  });
  ...
  broker.publishMessage = function (key, message, meta, debugMeta) {
    ...
    return new Promise((resolve, reject) => {
      ...
      broker.publish(key, message, meta, (brokerErr) => {
        ...
        logger.info(
          `Published message with routingKey: ${key} message: ${JSON.stringify(message, getCircularReplacer())} after ${
            (end - start) / 1000
          } seconds`,
          debugMeta
        );
