  ...
  broker.publishMessage = function(key, message, meta) {
    return new Promise((resolve, reject) => {
      broker.publish(key, message, meta, (brokerErr) => {
        if (brokerErr) {
          logger.emergency(
            `Failed to publish message with routingKey ${key}, error: ${brokerErr.toString()} message: ${message}`,
            {meta: {correlationId: message.meta.correlationId}}
          );
          return reject(brokerErr);
        }
        logger.info(`Published message with routingKey: ${key} message: ${message}`, {
          meta: {correlationId: message.meta.correlationId}
        });
        resolve();
      });
